...
///   ベース**を履いているときに Core も出すと、両側から同じ 5V レールを駆動する
///   ことになり、バッテリーで吸収できない CoreS3 SE では PoE 単独給電で起動
///   できなくなる。工場出荷ファームが PoE で動くのは BUS_EN を立てないため
///
/// **呼ぶのは hub-ui の i2c ループだけ** (i2c の所有者)。起動時は出さない。
/// 出すかどうかの判定は呼び手が持ち、規則は純関数
/// `alc_hub_core::usb5v::bus5v_sample` 1 本に畳んである — **このモジュールの
/// 契約 (渡された値をそのまま BUS_EN に書く) は変えていない**。
///
/// 既定 (`BUS5V AUTO`) は **USB ホスト (PC) が列挙されていて、かつ M-Bus が
/// 外部給電でない (`HubStatus::bus_in == Some(false)`) ときだけ出す**
/// (#202, Refs #211)。USB が居て M-Bus も外部給電でなければ VBUS がレールを
/// 支え、PC が落ちれば Core は手を引いてベース側に任せる。
/// **PoE のベースを履いた常設機は `BUS5V OFF`** に設定して固定する — #203 で
/// この設定を廃止したところ、現場が `OFF` にしていた NVS の値が無視され、
/// PoE 単独給電で起動しなくなった (#254)
///
