...
//! | `AUTH SIGN <nonce>` | nonce (小文字 hex 32 文字) にその 32 バイトそのもので署名し `AUTH SIG <pubkey base64url> <sig base64url>` を返す (鍵が無ければ `ERR AUTH: no key`、nonce の形式不正は `ERR AUTH: bad nonce`)。**管理者ログイン (`/auth/device-login`) がこの署名を使う — 署名対象にも応答の語数にも何も足さないこと** |
