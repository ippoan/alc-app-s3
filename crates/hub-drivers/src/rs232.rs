...
//! 本リポジトリでは扱わない。
//!
//! 実機 DIP 設定 (plan/cores3-hub-consolidation.md で確定):
//! - RXD DIP: 2 番 (シルク 16 = バスpin 16 = CoreS3 G17=PC_TX) を ON → RX=G17…ではなく
//!   ホスト TX=G17 がモジュール RX へ。
//! - TXD DIP: 3 番 (シルク 15 = バスpin 15 = CoreS3 G18=PC_RX) を ON → モジュール TX が
//!   ホスト RX=G18 へ。
//! - 結果、ホスト側は TX=G17 / RX=G18 (下記の Gpio17/Gpio18 と一致、他スイッチ全 OFF)。
