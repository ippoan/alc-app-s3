...
///
/// auth-worker /device/setup からの**遠隔**設定用 (シリアルの `BUS5V` と同じ NVS
/// 保存先)。遠隔で設定できることがこの機能の要点 — **PoE の現場の端末は
/// 手の届かない場所にあり、`OFF` にするために現地でシリアルを繋ぐ運用は
/// 成り立たない** (#254)。反映は即時 (hub-ui の i2c ループが 1 秒ごとに読む)。
///
/// ★**`on` を PoE のベースを履いた機へ送らないこと** — 同じ 5V レールを両側から
/// 駆動し、その機は PoE 単独で起動できなくなる (現地へ行く羽目になる)。
///
