...
pub struct HubStatus {
    ...
    /// 血圧計がボンドされているか (Refs #249)。**設定ではなく観測値** —
    /// hub-ble がスキャンのループごとに「NVS `bp_bond` の記録が NimBLE の
    /// ボンド一覧にまだ居るか」を計算して写す (真偽は永続化しない)。
    ///
    /// `AUTH SIGNBP` の署名対象に載る値で、鍵で裏付けられた「この端末に血圧計が
    /// 繋がっているか」の申告になる (管理者ログインが使う `AUTH SIGN` には載せない)。
    /// BLE を積まない機・`OMRON BP OFF` で BLE を起こさない機では false のまま =
    /// 「血圧計なし」で正しい
    pub bp_bonded: bool,
