...
マイナ免許証の免許情報読み取り (警察庁「マイナ免許証読み取りアプリ」で確認済みの事実):

- IC に記録されるのは **有効期間の末日・免許の種類/条件・色区分・免許情報記録の番号**等。
- アクセスは**マイナ免許証用の 4 桁暗証番号**だが、**未設定の場合は入力なしで利用可**
  (アプリに「暗証番号を設定していない場合」の分岐が公式に存在)。
- → 券面AP (照合番号B) と組み合わせれば、**マイナ免許証 1 枚で
  「顔写真 + 本人属性 + 免許有効期限」を PIN 手入力なしで取得**する構成が狙える。
  免許情報 AP の APDU 仕様は非公開のため要調査 (公式読み取りアプリの通信解析 or
  仕様書開示待ち)。当面、有効期限等はアプリ (スマホ側) で取得する。
