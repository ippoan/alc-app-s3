...
|---|---|---|
| 用途 | 営業所常設。NFC タップ → 打刻音 → 打刻イベント送信 | **運行管理者 PC 常設**。管理者のブラウザが閉じていること・**着信** (点呼の呼び出し) を音で知らせる (訂正 2026-09-09、§4) |
| ハード | Atom VoiceS3R (C126-ECHO) + Atomic PoE Base (A091) + Unit NFC (U216) | ATOM 系 + USB で**運行管理者 PC** に接続 (給電も USB) (訂正) |
| 通信 | PoE 1 本 (W5500) + 既存 WS 常時接続 | **USB のみ** (給電 + heartbeat)。着信も heartbeat に相乗り (案 B 本採用、§4.2 訂正) |
...
| 警告 (沈黙 = 繋がっていない) | 3000Hz 60ms ×2、**5 秒ごと** — 着信 / NG は上の 3 連のまま (2026-09-09 変更、`SILENCE_TICK_MS`) | (2) |
...
書いている (`ippoan/alc-app#168`)。**この端末には当てはまらない。**
別 repo なので #151 では触らず、別 issue で追跡する。
**ES8311 が入るまで、端末の現場向けフィードバックは 0 になる** (serial ログのみ)。

**実害が無いことは確認済み** (2026-09-05): 本番で `kind=timecard` を送った実績は
**通算 1 台・6 件だけ**で、すべて 2026-09-04 のベンチ検証。`#137` でインストーラを
Pages に載せてはいるが、**現場で動いている AtomS3 Lite の群は存在しない**。
したがってボード対応を差し替えても OTA で取り残す機は無い。
...
#### オフライン打刻の保持件数 — **flash がキュー本体、RAM は送信窓 (#142)**

> **2026-09-05 に「`MAX_QUEUE = 20` では足りない」として起票し、2026-09-09 に
> 実装した (#142)。** §3.3 冒頭に以前あった「`MAX_QUEUE = 20`。オフライン打刻の
> 取りこぼし対策はこれで済む」は誤りだった。

旧構成では未 ack の測定を**既定 `nvs` パーティションの文字列 1 キー**
(`ws_queue`) に改行区切りで持ち、読み出しバッファ 4096 に対し打刻 1 件が約
175 バイトだったため **20 件が保存方式のほぼ天井**だった。溢れると最も古い測定
から黙って捨てられ、**捨てられるのが打刻だと賃金計算のデータが欠ける**。

現在の構成:

- **キューの本体は専用 NVS パーティション `punchq` (512 KB) に 1 件 1 キー**
  (キー = seq の 16 進)。push = 1 キー書き、ack = 1 キー消しで、旧方式の
  「1 件積むたびに全件を書き戻す」コストは無い。打刻 (1 件 175 B = NVS 上
  224 B) 基準で **約 2,280 件** — 乗務員 20 名 × 出退勤で 57 日ぶん。
  点呼の測定 (session_id 込みで 250 B 前後) や crash_log が混ざると減る
- **RAM に載るのは送信窓 20 件と seq の索引だけ** (`ws_uplink.rs` の
  `WINDOW`)。窓が空けば flash から次を読むので、**保持件数を増やしても RAM は
  増えない** — PSRAM の有無を見る必要が無い (ベンチ機の AtomS3 Lite は
  `free_psram=0`)
- **`punchq` が無い機は既定 `nvs` の文字列 (上限 20 件) へフォールバックする**。
  **パーティションテーブルは OTA では更新されない**ので、OTA だけで上げた機は
  この経路に入る (`EVT PUNCHQ legacy`)。USB の Web インストーラーで焼き直すと
  新テーブルが入る
- 旧 `ws_queue` に未送信ぶんが残っていれば、punchq を開いた初回に seq を保った
  まま移す (`EVT PUNCHQ migrated <n>`)
- 溢れたときは従来どおり最古を捨てて新しい方を残す。ログは
  `EVT WS_DROPPED <seq> <kind>` で、**捨てたのが打刻かどうかが分かる**
- 電断時の意味が変わった: 旧 = 「全件書き戻るか全件旧状態」、
  新 = 「書き途中の 1 件だけ欠け得る」(NVS のキー書きは原子的) で安全側
- **溜まった分の排出は ack 駆動で連続する**: ack で窓に次のぶんが載ったら
  再送周期 (15 秒) を待たずに送る。1 往復 = 窓 20 件の送信 + その ack なので、
  排出速度は往復遅延だけで決まり、件数に比例して待たされることはない
  (この扱いをしないと 2,000 件で 25 分かかる)。**即時送信で送るのは窓のうち
  未送信ぶんだけ** — 窓の全件を送ると ack 1 件ごとにまだ ack 待ちの最大 19 件も
  送り直すことになり、2,000 件の排出で約 2 万通の重複になる。送信済みの印は
  RAM だけに持ち (flash には書かない)、再接続時に落として全件送り直す
...
> **仕様の訂正 (2026-09-09)** — ユーザー指示で**監視対象が変わった**。
> §4.1 / §4.2 の本文は**当初の設計 (キオスク PC の入力デバイス監視)** のまま残し、
> 矛盾する箇所に (訂正) を添えてある。
>
> - **置き場所**: 点呼キオスク (乗務員 PC) ではなく**運行管理者の PC**。USB でつなぐ
> - **監視対象**: FC-1200 / NFC ブリッジの生死ではなく、**運行管理者のブラウザ**
>   (alc-app トップ画面の「運行管理者」タブ) が開いているか
> - **鳴る条件は 3 つ**: (1) 管理者画面が閉じている / 別タブに居る → heartbeat が
>   止まり 10 秒で沈黙警告、(2) **着信** — 乗務員がキオスクで点呼を始めて signaling の
>   room が立った (`HB OK call=1`)。管理者が遠隔点呼に入ると止まる、
>   (3) `HB NG signaling` — 管理者のブラウザが signaling の `/watch-rooms` に
>   繋がっておらず**着信を受けられない**
> - **firmware は変わらない** (#178 / #179 / #182)。沈黙判定・`HB` / `STATUS` / `EVT` の
>   プロトコルも閾値もそのまま。変わったのは**誰が heartbeat を送り、`NG` の理由に
>   何を入れるか**だけ。ブラウザ側は `ippoan/alc-app` で並行実装中

**訂正後の用途 (2026-09-09)** — 運行管理者のブラウザ監視と着信の 2 つ:

- **運行管理者の画面が開いていない** — 管理者が「運行管理者」タブを閉じた / 別タブへ
  移った / ブラウザや PC が落ちた。heartbeat が途切れるので沈黙として鳴る
- **着信** — 乗務員がキオスクで点呼を始め、signaling に room が立った (`HB OK call=1`)。
  管理者が遠隔点呼に入ると止まる。購読自体が切れているときは `HB NG signaling`

以下は当初の記述 (経緯として残す):

点呼キオスク (ブラウザ) の異常を人に気付かせる据置ブザー。用途は 2 つ: **(訂正)**
...
- **キオスクの入力デバイスが切れた** — FC-1200 (WebSerial,
  ...
  **(訂正 — この用途は無くなった。入力デバイスの生死は見ない)**
- **点呼の呼び出しが来ている** **(訂正 — 「着信」として残った。源は §4.2 を見る)**
...
heartbeat の中身は最小限にする。**送り手が自分で判断した結果** (正常か否か) を
(訂正 — 送り手は運行管理者のブラウザ)
...
> **(訂正 2026-09-09)** 送るのは**運行管理者のブラウザ**で、`OK` / `NG` の中身も
> 変わった。上の 2 行は当初の例。行の**書式は変えていない**ので firmware は同じ。
>
> ```
> HB OK call=0     … 管理者画面が開いている。着信なし
> HB OK call=1     … 着信 (乗務員が点呼を始めて room が立った) → 鳴る
> HB NG signaling  … 管理者のブラウザが signaling の /watch-rooms に繋がっていない
> ```
...
**USB 給電なので PC (訂正 — 運行管理者 PC) の電源が落ちるとブザーも死ぬ**。ユーザー判断で**許容**
...
> **(訂正 2026-09-09) 案 B (heartbeat 相乗り) が本採用。着信の源は signaling の
> `/watch-rooms`。** 運行管理者のブラウザがここを購読していて、乗務員がキオスクで
> 点呼を始めると room が立つ。ブラウザはそれを見て `HB OK call=1` を送り、
> 警告デバイスが鳴る。管理者が遠隔点呼に入って room に参加すると `call=0` に戻って
> 止まる。購読が切れているあいだは `HB NG signaling` を送るので、
> **着信を受けられない状態そのもの**も鳴って伝わる。
>
> **端末は IP にもクレデンシャルにも触らない**ので、下の案 A (Wi-Fi + WS の下り
> command) は**採らない**。以下の本文は当初の検討として残す。
...
ハード構成は「ATOM 系 + USB でキオスク PC に接続 (給電も USB)」(訂正 — 運行管理者 PC) なので、
...
**(訂正 2026-09-09 — B を本採用とし、A には進まない。上の blockquote)**
...
止めた事実は `EVT` としてホスト (訂正 — 運行管理者 PC のブラウザ) へも出し、ブラウザ側で「ブザーを人が
止めた」ことが分かるようにする。**異常が解消していない限り、次の周期でまた鳴らす
ようなことはしない** (人が認識済みなので)。異常が一度解消して再発したら、また鳴る。

> **2026-09-09 の実機確認で変更** (上の段落は経緯として残す)。実機で鳴らした
> ユーザーの要望により、次の 2 点を足した:
>
> 1. **ボタンはトグル** — 黙らせた状態でもう一度押すと鳴動へ戻る
>    (`Muted ──ボタン──▶ Alarming`)。戻すときは押した手応えとして即 1 回鳴らし、
>    次の周期を `now + ALERT_PERIOD_MS` に置く。「黙らせたら異常が解消するまで
>    鳴らない」だけだと、押し間違えた / もう一度確認したいときに戻す手段が無い。
> 2. **黙らせているあいだも `MUTED_TICK_MS` (5 秒) ごとに短い合図** を出す
>    (3000Hz 60ms ×1。鳴動の 3 連とは別音)。**完全な無音にすると異常が続いて
>    いることを忘れられる** — 「人が認識済み」は押した瞬間しか成り立たない。
>
> 実装は `crates/hub-core/src/alarm.rs` の `on_button` / `State::Muted`。
...
2. crate 雛形 + heartbeat 受信 + 沈黙検知 + 鳴動 + 停止ボタン
   (**ネットワークなし・USB だけで完結**)
3. `alc-app` 側: **運行管理者のブラウザ**から heartbeat を送る (§4.1、訂正 2026-09-09)。
   §4.2 案 B (呼び出し状態を heartbeat に相乗り) までをここで出す
4. Wi-Fi + WS 常時接続 + 下り `alert` command (§4.2 案 A) — **(訂正 2026-09-09: 進まない)**
