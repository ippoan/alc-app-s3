...
| `BUS5V AUTO\|ON\|OFF` | M-Bus 5V を Core 側から出すかの設定 (NVS 保存、既定 `AUTO`。Refs #254)。**`OFF` = 絶対に出さない — PoE のベース (Base LAN PoE v1.2 等) を履いた常設機はこれ**。ベースが自前で M-Bus 5V を供給しているところへ Core も出すと、同じ 5V レールを両側から駆動することになり、バッテリーで突入を吸収できない CoreS3 SE は **PoE 単独給電で起動できなくなる**。`AUTO` = USB ホスト (PC) が列挙されていて、かつ M-Bus が外部給電でない (`BUS_IN=0`) 間だけ出す (#202 の固定動作)。`ON` = 常に出す — USB 電源アダプタ (PC ではないので USB ホストとして列挙されず `AUTO` では出ない) で動かすベンチに RS232M / LAN 13.2 を積む構成向け (Refs #76)。★**`ON` を PoE の機で使わないこと** (#203 は実機で「`on` は PC 再起動中に PoE 単独で起動できない」を確認している)。反映は即時 (hub-ui が 1 秒ごとに設定を読む) で再起動は要らない。**遠隔からは WS 下り command `{action:"bus5v","mode":"auto\|on\|off"}`** (応答 `{ok,mode}`) で同じ設定を変えられる — 現場の端末にシリアルを繋がずに `OFF` にできる (`1`/`0` は WS では受けない) |
| `BUS5V STATUS` | M-Bus 5V の設定と現況 `BUS5V MODE=auto USB=1 OUT=1 BATTERY=0 BUS_IN=0` を返す。`MODE` が上の設定、`OUT` が**いま実際に出しているか**。hub-ui の i2c ループが 1 秒ごとに `MODE` と USB ホストの有無を読んで追随する (起動時は出さない / 起動から 3 秒は読まない / 同じ値が 2 回続いてから切り替える)。`BUS_IN` は起動時の W5500 probe を材料に決まる (`1`=PoE 等で外部給電中 `0`=無し `?`=未判定、Refs #211) — `AUTO` では `BUS_IN` が `1`/`?` の間は切り替え自体を起こさない。**probe の失敗では `0` にしない** (起動直後は W5500 がまだ立ち上がっていないことがあり、1 回勝負で `0` に誤確定すると PoE 単独給電で起動できなくなる、#254) — 起動から 15 秒 (`usb5v::BUS_IN_GRACE_MS`) 経っても `?` のままなら `0` に確定し、そのとき `EVT BUS_IN=0 …` を 1 行出す。切り替え時に `EVT BUS5V OUT=<0\|1> usb_host=<0\|1> mode=<auto\|on\|off>` を出す。WS 下り command `{action:"bus5v_status"}` (応答 `{mode,usb_host,ext_5v_out,battery_present,power_read,bus_in}`) / `{action:"reboot"}` (OTA 中・点呼中は `{ok:false,message:"busy"}`) で、auth-worker の端末一覧から遠隔で照会・再起動できる |
...
- 起動時に内部 I2C を probe (RTC 0x51 / IMU 0x69) して板種別を判定し、`DEVICE … BOARD=`
