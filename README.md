...
| `GW URL <ws://...>` | Windows GW (alc-gw) ハブの URL 保存 (NVS)。設定すると測定を GW へ生中継する。WS 下り command `{action:"gw_url",url}` / `{action:"gw_status"}` (auth-worker /device/setup) でも遠隔で設定・確認できる |
| `GW STATUS` | `GW CONNECTED=1 URL=ws://192.168.11.5:9000` 応答 |
